...
- 同步合併出新紀錄時（如第二台電腦首次同步），該輪成功後補抓缺漏的物品圖示與詳情，走與「更新物品資料」相同的進度對話框讓使用者看到狀態，結束顯示物品資料／補圖張數摘要（不帶匯入結果文案）；**沒有實際補到任何東西時全程靜默、不彈完成訊息**（補圖本身 best-effort、失敗僅記 log；忙碌中略過，留待下次「更新」補齊）。
